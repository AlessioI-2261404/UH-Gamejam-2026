...
@onready var leveldone = $L

#@onready var box = $Box

#Key-gate relations
@onready var key1 = $key
@onready var gate1 = $gate/CollisionShape2D
...
func _ready() -> void:
	...
	hud.set_initial_pencil_power(2750)	
...
func _clear_drawer_path() -> void:
	...
	key1.show()
	gate1.set_deferred("disabled", false)
...
func _on_exit_body_entered(body: Node2D) -> void:
	if body.name == "Player":
		...
		if next_scene != "":
			...
			hud.play_level_finished_sound(3)
